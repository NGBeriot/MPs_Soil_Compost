...
#Loading data ####
  ...
  wd="C:/Users/berio001/OneDrive - WageningenUR/PhD/Field Assessment/Benjamin/Submission" #C:/Users/Directory# Write the directory with SAS_results.xlsx and Raw_data_Beriot.xlsx
  setwd(wd)
...
# Plots ####
  ...
  #*** Fig.2 Microplastic content  ####
    ...
    PLOT2= ggplot()+
      ...
      geom_text(data = Treat_layer_summary_R ,aes(x=Treat_Layer, y= Q1+150, label=Sign.letter),
...
# *** Fig.S1 Density size distribution ####
  ...
  # data.frame Fig.S1 : P_density
    ...
    PLOT3=ggplot( P_density, aes(x=Area, fill=Treat_Layer)) + 
      geom_density(alpha=.3)+
      facet_wrap(~Treat) +
      scale_fill_manual(values = my_palette )+
      xlab(expression("Particles area [mm"^2 *"]"))+
      ylab("Density [%]")+
      theme(
        plot.title = element_blank(),
